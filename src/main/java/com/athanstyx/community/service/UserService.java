...
import com.athanstyx.community.util.MailClient;
...
public class UserService implements CommunityConstant {
    ...
    //因为注册是需要用到User所以把注册的服务写在这里
    //注册的时候需要发邮件, 邮件客户端
    @Autowired
    private MailClient mailClient;
    //注册时候发邮件需要发html格式的所以需要模版引擎
    @Autowired
...
//    private LoginTicketMapper loginTicketMapper;
    ...
    //因为注册时候需要发送激活链接所以需要域名和项目名
    //域名(application.properties)
...
//        return userMapper.selectById(id);
    ...
    public Map<String, Object> login(String username, String password, int expiredSeconds) {
        ...
        System.out.println(CommunityUtil.MD5("123"+"28699"));
...
//        loginTicketMapper.updateStatus(ticket, 1);
    ...
    public int updatePassword(int userId, String passwordNew){
        int rows = userMapper.updatePassword(userId, passwordNew);
        clearCache(userId);
        return rows;
    }
